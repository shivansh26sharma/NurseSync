...
Then open index.html (e.g. via VS Code Live Server) and click Record.
"""
...
def get_model():
    ...
    the model is downloaded once, no internet needed either.
    """
    global _model
    if _model is None:
...
async def transcribe(audio: UploadFile = File(...)):
    ...
    try:
        model = get_model()
        segments, _info = model.transcribe(str(dest), beam_size=5)
